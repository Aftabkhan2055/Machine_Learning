...
class sellq:
    def date(self):
        ...
        query = "select * from booking where dateofentry='" + self.fromdate.get() + "' and '" + self.todate.get() + "'"
        ...
        query1="SELECT price from booking where dateofentry='" + self.fromdate.get() + "' and '" + self.todate.get() +"'"
        cd=dr.conn.cursor()
        cd.execute(query1)
        q=cd.fetchall()
        s=0
    ...
    def __init__(self):
        ...
        #self.m = StringVar()
        lb = Label(down, text="From ", font=("Arial", 12, "bold"), bg="light green", fg="brown").grid(row=0, column=1,sticky=(E))
        lb = Label(down, text="To ", font=("Arial", 12, "bold"), bg="light green", fg="brown").grid(row=0, column=2,sticky=(E))
        self.fromdate = Entry(down)
        self.todate = Entry(down)
        self.fromdate.grid(row=0, column=2, sticky=(W))
        self.todate.grid(row=0, column=3, sticky=(W))
